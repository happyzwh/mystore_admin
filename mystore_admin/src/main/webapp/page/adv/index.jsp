	...
	<head>
	    ...
	    <script  type="text/javascript">
	       ...
	       function addNote(){
	           ...
	           if($.trim($("#type").val()) !='0' &&  $.trim($("#type").val()) != '1'){
	              if($.trim($("#bh").val()) == ''){
		              alert("提示：编号不能为空！");
		              return false;
	              }
	              var patrns=/^[A-Z]{2}\d{4}[1-9]$/; 
			      if(!patrns.exec($.trim($("#bh").val()))){
						alert("提示：编号格式应为两个大写英文字母开始后随5位数字结束！");
	                    return false;
				  }
	           }
	           ...
	           $.ajax({
					url: '<%=path%>/adv/advAction!addNote.dhtml',
					type: 'post',
					data: {'name':$.trim($("#name").val()),'type':$.trim($("#type").val()),'bh':$.trim($("#bh").val()),
	       ...
	       function deleteNote() {
	           ...
	           if(confirm("提示：确定删除！")){
		           $.ajax({
						url: '<%=path%>/adv/advAction!deleteNote.dhtml',
						type: 'post',
	       ...
	       function updateNote(){
	           ...
	           $.ajax({
					url: '<%=path%>/adv/advAction!updateNote.dhtml',
					type: 'post',
					data: {'name':$.trim($("#name").val()),'type':$.trim($("#type").val()),'bh':$.trim($("#bh").val()),
					      'sort':$.trim($("#sort").val()), 'descr':$.trim($("#descr").val()), 
...
<body class="main">
	 <div class="location"><span>当前位置：广告分类管理</span></div>
	 <div class="main">
						           ...
						           <tr>
						               ...
						               <td width="50%" style="text-align:left;">
						                 <select id="type">
						                     ...
						                     <option value="1">页面分类</option>
						                     <option value="2">广告位置</option>
						                     <option value="3">商品模块</option>
						                     <option value="4">文字图片模块</option>
						                 </select>
						               </td>
						           </tr>
						           <tr>
