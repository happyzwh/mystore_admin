...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
	    ...
	    <script  type="text/javascript">
	     ...
	     function addPro(){
			 location.href='<%=path%>/adv/advAction!edit_pro.dhtml?pid='+$("#pid").val();   
		 }
		 function updatePro(id){
			 location.href='<%=path%>/adv/advAction!edit_pro.dhtml?id='+id;   
		 }
		 function delPro(id){
		      if(confirm("提示：是否确认删除？")){
			       $.ajax({
								url: '<%=path%>/adv/advAction!delProModule.dhtml',
								type: 'post',
								data: {'id':id},
								async: false,
								dataType: "text",
		 ...
		 function save(){
			          ...
			          if(attrArr.length == 0){
				          ...
				          $.ajax({
									url: '<%=path%>/adv/advAction!saveProModuleSort.dhtml',
									type: 'post',
	 ...
	 <div class="main" style="margin-bottom:20px;">
        <table class="table_list" cellpadding="0" cellspacing="0" style="margin-top:0px;">
           <tr class="head">
            ...
            <tbody id="tbody">
            <s:iterator value="proMoudleList" id="pro" status="index">
