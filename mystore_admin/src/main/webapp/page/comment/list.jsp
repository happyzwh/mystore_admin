...
<body class="main">
	 <div class="location"><span>当前位置：评论列表</span></div>
	 <div class="main" style="margin-bottom:20px;">
        <!--按条件查询-->
        <form id="queryForm" action="<%=path%>/product/commentAction!list.dhtml" method="post">
	               ...
	               <td width="35%">
	                    <select id="status" name="status" style="width:100px;">
	                        <option value="">全部</option>
	                     	...
	                     	<option <s:if test='#request.status == "2"'>selected="selected"</s:if> value="2">审核不通过</option>
	                    </select>
	               </td>
        ...
        <table class="table_list" cellpadding="0" cellspacing="0" style="margin-top:0px;">
           <tr class="head">
            ...
            <s:iterator value="list" id="comment" status="index">
	           <tr>
	               <td><s:property value="#comment.userName"/></td> 
	               <td><s:property value="#comment.productName"/></td>
	               <td>
	               ...
	               <td><s:date name='#comment.createDate' format="yyyy-MM-dd HH:mm:ss" /></td>   
	               <td><s:date name='#comment.lastDate' format="yyyy-MM-dd HH:mm:ss" /></td>   
	               <td>
	                   <power:permission bh="view_comment">
	                   		<a href='<%=path%>/product/commentAction!detail.dhtml?id=<s:property value="#comment.id"/>' target="_self">详情</a>
	                   </power:permission>
	               </td>
	           </tr>
           </s:iterator>
